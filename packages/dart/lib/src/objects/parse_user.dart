...
class ParseUser extends ParseObject implements ParseCloneable {
  ...
  ParseUser.clone(Map<String, dynamic>? map)
      : this(map != null ? map[keyVarUsername] : null, null, map != null ? map[keyVarEmail] : null);

  @override
  dynamic clone(Map<String, dynamic>? map) => ParseUser.clone(map)..fromJson(map);
  ...
  set emailVerified(bool? emailVerified) => set<bool>(keyEmailVerified, emailVerified);
  ...
  set sessionToken(String? sessionToken) => set<String>(keyVarSessionToken, sessionToken);

  Map<String, dynamic>? get authData => super.get<Map<String, dynamic>>(keyVarAuthData);

  set authData(Map<String, dynamic>? authData) => set<Map<String, dynamic>>(keyVarAuthData, authData);

  static ParseUser createUser({String? username, String? password, String? emailAddress}) {
    return ParseCoreData().createParseUser(username, password, emailAddress);
  }
  ...
  static Future<ParseResponse?> getCurrentUserFromServer(String token, {bool? debug, ParseClient? client}) async {
  ...
  Future<ParseResponse?> getUpdatedUser({bool? debug, ParseClient? client}) async {
    ...
    final ParseClient _client =
        client ?? ParseCoreData().clientCreator(sendSessionId: true, securityContext: ParseCoreData().securityContext);
    ...
    try {
      ...
      return await _handleResponse(this, response, ParseApiRQ.currentUser, _debug, parseClassName);
  ...
  Future<ParseResponse?> signUp({bool allowWithoutEmail = false, bool doNotSendInstallationID = false}) async {
    forgetLocalSession();
    ...
    try {
      if (emailAddress == null) {
        if (!allowWithoutEmail) {
          ...
          assert(() {
            print('It is recommended to only allow user signUp with an email set.');
            return true;
          }());
        }
      }
      ...
      final ParseNetworkResponse response = await _client.post(url.toString(),
          options: ParseNetworkOptions(headers: <String, String>{
            ...
            if (installationId != null && !doNotSendInstallationID) keyHeaderInstallationId: installationId,
          }),
          data: body);

      return await _handleResponse(this, response, ParseApiRQ.signUp, _debug, parseClassName);
    } on Exception catch (e) {
  ...
  Future<ParseResponse?> login({bool doNotSendInstallationID = false}) async {
    ...
    try {
      final Map<String, dynamic> queryParams = <String, String?>{keyVarUsername: username, keyVarPassword: password};
      final String? installationId = await _getInstallationId();
      ...
      return await _handleResponse(this, response, ParseApiRQ.login, _debug, parseClassName);
    } on Exception catch (e) {
  ...
  /// Set [doNotSendInstallationID] to 'true' in order to prevent the SDK from sending the installationID to the Server.
  /// This option is especially useful if you are running you application on web and you don't have permission to add 'X-Parse-Installation-Id' as an allowed header on your parse-server.
  Future<ParseResponse?> loginAnonymous({bool doNotSendInstallationID = false}) async {
    forgetLocalSession();
    try {
      ...
      return await _handleResponse(this, response, ParseApiRQ.loginAnonymous, _debug, parseClassName);
    } on Exception catch (e) {
  ...
  static Future<ParseResponse?> loginWith(
    ...
    final ParseUser user = ParseUser.createUser(username: username, password: password, emailAddress: email);
    final ParseResponse? response =
        await user._loginWith(provider, authData, doNotSendInstallationID: doNotSendInstallationID);
    return response;
  }

  /// Set [doNotSendInstallationID] to 'true' in order to prevent the SDK from sending the installationID to the Server.
  /// This option is especially useful if you are running you application on web and you don't have permission to add 'X-Parse-Installation-Id' as an allowed header on your parse-server.
  Future<ParseResponse?> _loginWith(String provider, Object authData, {bool doNotSendInstallationID = false}) async {
    try {
      final Uri url = getSanitisedUri(_client, '$keyEndPointUsers');
      final String? installationId = await _getInstallationId();
      ...
      return await _handleResponse(this, response, ParseApiRQ.loginWith, _debug, parseClassName);
    } on Exception catch (e) {
  ...
  Future<ParseResponse?> logout({bool deleteLocalUserData = true}) async {
    ...
    try {
      ...
      final ParseNetworkResponse response = await _client.post(
        url.toString(),
        options: ParseNetworkOptions(headers: <String, String>{keyHeaderSessionToken: sessionId!}),
      );

      return await _handleResponse(this, response, ParseApiRQ.logout, _debug, parseClassName);
    } on Exception catch (e) {
  ...
  Future<ParseResponse?> verificationEmailRequest() async {
    try {
      ...
      return await _handleResponse(this, response, ParseApiRQ.verificationEmailRequest, _debug, parseClassName);
    } on Exception catch (e) {
      return handleException(e, ParseApiRQ.verificationEmailRequest, _debug, parseClassName);
    }
  }
  ...
  Future<ParseResponse?> requestPasswordReset() async {
    try {
      ...
      return await _handleResponse(this, response, ParseApiRQ.requestPasswordReset, _debug, parseClassName);
    } on Exception catch (e) {
      return handleException(e, ParseApiRQ.requestPasswordReset, _debug, parseClassName);
    }
  }
  ...
  Future<ParseResponse?> destroy() async {
    if (objectId != null) {
      try {
        ...
        final ParseNetworkResponse response = await _client.delete(url.toString());
        return await _handleResponse(this, response, ParseApiRQ.destroy, _debug, parseClassName);
      } on Exception catch (e) {
  ...
  static Future<ParseResponse?> all({bool? debug, ParseClient? client}) async {
    ...
    try {
      ...
      final ParseResponse? parseResponse =
          handleResponse<ParseUser>(emptyUser, response, ParseApiRQ.getAll, _debug, keyClassUser);
  ...
  static Future<dynamic> _getUserFromLocalStore({ParseCloneable? cloneable}) async {
  ...
  static Future<ParseResponse?> _handleResponse(
      ParseUser user, ParseNetworkResponse response, ParseApiRQ type, bool debug, String className) async {
    final ParseResponse? parseResponse = handleResponse<ParseUser>(user, response, type, debug, className);
  ...
  static ParseUser _getEmptyUser() => ParseCoreData().createParseUser(null, null, null);

  static Future<String?> _getInstallationId() async {
    final ParseInstallation? parseInstallation = await ParseInstallation.currentInstallation();
