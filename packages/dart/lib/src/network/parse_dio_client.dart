...
class ParseDioClient extends ParseClient {
  ...
  Future<ParseNetworkResponse> postBytes(String path,
      {Stream<List<int>>? data, ParseNetworkOptions? options, ProgressCallback? onSendProgress}) async {
    try {
      final dio.Response<String> dioResponse = await _client.post<String>(
        path,
        data: data,
...
class _ParseDioClient with DioMixin implements dio.Dio {
  _ParseDioClient({
    bool? sendSessionId,
    SecurityContext? securityContext,
  ...
  Future<dio.Response<T>> request<T>(
    ...
    if ((_sendSessionId == true) && (this.data.sessionId != null) && (options.headers![keyHeaderSessionToken] == null))
  ...
  void _logCUrl(dio.Options options, dynamic data, String url) {
    if (options.method == null || options.headers == null) {
      print('Unable to log options: $options');
      return;
    }
