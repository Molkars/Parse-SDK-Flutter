...
import 'package:parse_server_sdk/src/network/parse_websocket.dart' as parse_web_socket;
...
class Parse {
  ...
  Parse._(this._hasBeenInitialized);
  ...
  factory Parse(
    ...
    ParseCoreData.init(
      ...
      clientCreator: clientCreator ?? _defaultClientCreator,
    );
  ...
  Future<ParseResponse?> healthCheck({
    ...
    final ParseClient _client = client ??
        ParseCoreData().clientCreator(
            sendSessionId: sendSessionIdByDefault ?? ParseCoreData().autoSendSessionId,
    ...
    try {
      final ParseNetworkResponse response = await _client.get('${ParseCoreData().serverUrl}$keyEndPointHealth');
